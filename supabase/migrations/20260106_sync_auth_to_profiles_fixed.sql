/*
  # Sync auth.users to profiles table (FIXED)
...
SELECT
  ...
  au.email,
  CASE
    WHEN au.email = 'admin@celestar.com' THEN 'PLATFORM_ADMIN'
    WHEN au.email = 'program.owner@celestar.com' THEN 'PROGRAM_OWNER'
    WHEN au.email = 'workstream.lead@celestar.com' THEN 'WORKSTREAM_LEAD'
    WHEN au.email = 'field@celestar.com' THEN 'FIELD_CONTRIBUTOR'
    WHEN au.email = 'client@celestar.com' THEN 'CLIENT_VIEWER'
    ELSE 'CLIENT_VIEWER'  -- Default role for any other users
...
SET
  role = CASE
    WHEN email = 'admin@celestar.com' THEN 'PLATFORM_ADMIN'::app_role
    WHEN email = 'program.owner@celestar.com' THEN 'PROGRAM_OWNER'::app_role
    WHEN email = 'workstream.lead@celestar.com' THEN 'WORKSTREAM_LEAD'::app_role
    WHEN email = 'field@celestar.com' THEN 'FIELD_CONTRIBUTOR'::app_role
    WHEN email = 'client@celestar.com' THEN 'CLIENT_VIEWER'::app_role
...
ORDER BY
  CASE role
    WHEN 'PLATFORM_ADMIN'::app_role THEN 1
    WHEN 'PROGRAM_OWNER'::app_role THEN 2
    WHEN 'WORKSTREAM_LEAD'::app_role THEN 3
    WHEN 'FIELD_CONTRIBUTOR'::app_role THEN 4
    WHEN 'CLIENT_VIEWER'::app_role THEN 5
  END;
